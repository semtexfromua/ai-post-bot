...
def test_filter_item_idempotent_on_redelivery(db, monkeypatch):
    """A redelivered filter_item (acks_late worker loss before ack) must not drop
    the item: calling it twice for the same news_id returns the news_id both times.
    Durable dedup lives in parse_source (content_hash UNIQUE), so the filter gate
    must be replay-safe."""
    item = _persist_news(db, "Сьогодні відбулося багато виборів", "h-redeliver")
    monkeypatch.setattr(pipeline, "SessionLocal", lambda: db_ctx(db))
    monkeypatch.setattr(pipeline, "_load_keywords", lambda s: [_FakeKw("вибори")])

    first = pipeline.filter_item.run(str(item.id))
    second = pipeline.filter_item.run(str(item.id))  # redelivery

    assert first == str(item.id)
    assert second == str(item.id)
