...
- **Flower runs app-less.** Importing `aiogram` installs the uvloop event-loop policy globally, which deadlocks Flower's tornado HTTP loop — with `-A app.tasks.celery_app` (whose `include` pulls in the aiogram-importing task module) every UI request hangs forever. The compose `flower` service therefore connects with `--broker` only, no app. Workers and the API are unaffected (prefork creates fresh loops; uvicorn supports uvloop natively).
