...
- **Flower needs tornado < 6.5.** Flower `2.0.1` (the latest release) never answers HTTP under tornado 6.5 — the UI on `:5555` hangs while logs look healthy. `pyproject.toml` pins `tornado>=6.4,<6.5`, with which the dashboard works; drop the pin once a newer Flower ships.
