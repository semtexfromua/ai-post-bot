...
uv run pytest -q
# Очікується: усі тести проходять (наразі 199 passed)
...
| `OPENAI_API_KEY` | Ключ OpenAI API або OpenRouter (SecretStr) | `sk-...` / `sk-or-...` |
| `OPENAI_MODEL` | Модель для генерації постів (на OpenRouter — id моделі зі structured outputs) | `gpt-4o-mini` |
...
| `MODERATION_ENABLED` | Гейт модерації; `false` для OpenRouter (нема `/moderations`) | `true` |
...
> Для ad-hoc тексту створюється синтетичний `NewsItem` (`source="manual"`),
> після чого виконується **лише генерація** (без авто-публікації) — як і для
> запиту за `news_id`. Результат з'явиться у `GET /api/v1/posts` після виконання.
...
| 5 | Parse RSS → NewsItem(title/url/summary/published_at UTC); повторний URL → UNIQUE content_hash = no-op | ✅ | `app/news_parser/feed.py`; `tests/parser/test_feed.py` |
...
| 9 | Pipeline wiring (task_always_eager): orchestrator → chain end-to-end на фейках | ✅ | `tests/tasks/test_pipeline_eager.py` |
...
> **Свідоме відхилення від ТЗ.** Оригінальне завдання (M4 §5) дослівно вимагає
> «публікація через **Telethon**». Ми навмисно публікуємо через **aiogram (Bot API)**,
> бо це знижує ban-ризик: публікувати від імені юзер-сесії (Telethon) — найризикованіша
> дія для акаунта (spam-репорти → блок), тоді як офіційний бот-адмін каналу публікує
> безпечно, а юзер-сесія лишається пасивним read-only читачем. Telethon усе одно
> використовується — для читання каналів, де Bot API безсилий. Деталі ризиків нижче:
> [Залишковий ban-ризик](#залишковий-ban-ризик-юзер-акаунту), [ToS §1.5](#tos-telegram-15).
